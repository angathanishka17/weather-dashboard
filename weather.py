...
open meteo api : to get he weather on  the latitude and longitude
'''
def geocode(city):
    r = requests.get(
...
def fetch_weather(lat,lon):
    r = requests.get(
        ...
        params={"latitude": lat,"longitude": lon,"current": "temperature_2m,apparent_tempature,relative_humidity_2m,"
                "wind_speed_10m,wind_direction_10m,precipitation,weathercode,uv_index","hourly":"temperature_2m,precipitation_probability",
...
city_input=st.text_input("enter a city name",placeholder="e.g.london,pairs,new york")
...
with st.spinner("Fetching weather data..."):
    try:
        data  = fetch_weather(loc["latitude"],loc["logitude"])
    except Exception as e:
...
st.metric("Temperature",f"fmt(cur['temperature_2m']){unit}",
          f"Fells like{fmt(cur['apparent_temperature'])}{unit}")
col1, col2, col3, col4= st.columns(4)
col1.metric(":cloudy: Humidity",f"{cur['relative_humidity-2m']}%")
col2.metric(":wind_blowing_face: Wind Speed", f"{cur['wind_speed-10m']}{wind_direction(cur['wind_direction_10m'])}")
